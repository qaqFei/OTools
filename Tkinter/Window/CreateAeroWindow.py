...
def _test():
    ...
    def f():
        ...
        t=not t
        if t:
            AeroWindow.title("Tkinter.Window.CreateAeroWindow"+str(int(AeroWindow.frame(),16)))
        else:
            AeroWindow.title("")
    AeroWindow = CreateAeroWindow(500,500,fullscreen=False,bar=True,resizable=(False,False))
    Label(AeroWindow,text="Hello World!",background="#000001",font=("",30)).pack()
    Button(AeroWindow,text="Hello World!",command=f).pack(pady=10)
    AeroWindow.deiconify()
    AeroWindow.mainloop()
